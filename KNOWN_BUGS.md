...
|----|--------|-------|---------|
| BUG-001 | FIXED | label | SUM (Summit Materials) delisted — removed from universe |
| BUG-002 | FIXED | unit | AVAV price target ~$148M/share (dollars↔$mm mismatch) |
| BUG-003 | OPEN | cohort | LMT/MA price target floored to $0, confidence=BROKEN |
| BUG-004 | FIXED | label | HON-class gross_profit=None for conglomerates |
| BUG-005 | FIXED | cohort | LLY-class peer multiples not representative |
| BUG-006 | FIXED | label | Health-insurer gross_profit inflation (UNH/HUM/CI/ELV/CVS) |
| BUG-007 | FIXED | plumbing | run_pipeline.py always bypasses live peer fetch |

### Bug classes

New bugs must be classified into one of these shared root-cause buckets on entry
(add the class to the index row **and** the bug's `Class:` line):

- **unit** — a value is off by a fixed numeric factor (e.g. dollars vs `$mm`, %
  vs fraction). Symptom is an answer that is right in shape but wrong by ~10^n.
- **label** — EDGAR/XBRL (or another source) reports a concept under a
  non-standard label, so extraction returns `None` or picks the wrong line item.
  Fix by expanding a label map and/or adding a computed fallback.
- **cohort** — peer-relative math (comps multiples, outlier detection) misfires
  because the target does not belong to the multiple regime of its assigned peer
  group. Fix by per-cohort calibration or outlier guards, not by touching the
  target's own numbers.
- **plumbing** — wiring/defaults/control-flow: the calculation is correct but the
  wrong inputs reach it (argparse defaults, guard conditions, path resolution).
